...
# only authors
select_query = "select data.uid, array_agg(data.item_id order by data.uid, data.item_id) entities from (select du.id as uid, item_id from dataprocessing_items items join workprogramsapp_prerequisitesofworkprogram prerequisites on items.id = prerequisites.workprogram_id join workprogramsapp_workprogram program on program.id = prerequisites.workprogram_id join public.workprogramsapp_expertise we on program.id = we.work_program_id join dataprocessing_user du on program.authors like '%' || du.last_name || '%' where items.domain_id is not null and du.first_name <> '' and du.last_name <> '' and we.expertise_status = 'AC') as data group by data.uid order by data.uid;"
# authors & editors
# select_query = "select * from (select data.uid, array_agg(data.item_id order by data.uid, data.item_id) entities from (select du.id as uid, item_id from dataprocessing_items items join workprogramsapp_prerequisitesofworkprogram prerequisites on items.id = prerequisites.workprogram_id join workprogramsapp_workprogram program on program.id = prerequisites.workprogram_id join public.workprogramsapp_expertise we on program.id = we.work_program_id join dataprocessing_user du on program.authors like '%' || du.last_name || '%' where items.domain_id is not null and du.first_name <> '' and du.last_name <> '' and we.expertise_status = 'AC') as data group by data.uid order by data.uid) as by_authors union select * from (select data.user_id, array_agg(data.item_id order by data.user_id, data.item_id) entities from (select * from dataprocessing_items items join workprogramsapp_prerequisitesofworkprogram prerequisites on items.id = prerequisites.workprogram_id join workprogramsapp_workprogram program on program.id = prerequisites.workprogram_id join workprogramsapp_workprogram_editors editors on program.id = editors.workprogram_id join public.workprogramsapp_expertise we on program.id = we.work_program_id where items.domain_id is not null and we.expertise_status = 'AC') as data group by data.user_id order by data.user_id) as by_editors;"
